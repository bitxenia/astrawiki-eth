...
describe("Article 10000 bytes", () => {
  ...
  test(
    "measure 1000 samples of time for newArticle with 10000 bytes",
    async () => {
      ...
      saveMetrics(durations, "newArticle10000_time");
      expect(durations.length).toBe(1000);

      const metrics = node.getMetrics();
      const results = metrics.getResults();
      saveMetrics(results.gasUsed, "newArticle10000_gasUsed");
    },
