...
describe("Article 10000 bytes", () => {
  ...
  it("measure time for newArticle with 10000 bytes", async function () {
  ...
  it("measure 1000 samples of time for getArticle with 10000 bytes", async function () {
    ...
    for (let i = 0; i < 1000; i++) {
      try {
        ...
        expect(article.name).to.equal(ARTICLE_NAME);
        expect(article.content.length).to.equal(10000);
      } catch (error) {
